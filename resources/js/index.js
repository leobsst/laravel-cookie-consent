...
function setCookieConsent(value) {
    let c = window.CookieConsent;
    let expires = new Date(Date.now() + c.duration * 1000).toUTCString();
    let cookie = c.cookieName + '=' + value + '; expires=' + expires + '; path=/; SameSite=' + c.sameSite;
    if (c.secure) cookie += '; Secure';
    document.cookie = cookie;
}

function updateGtag(granted) {
    if ((HAS_GTM || HAS_ADSENSE) && typeof gtag === 'function') {
        gtag('consent', 'update', {
    ...
    if (HAS_ADSENSE && window.__tcfapi && window.__tcfapi._notify) {
        window.__tcfapi._notify();
    }
}

function updatePosthog(granted) {
    if (!HAS_POSTHOG || !window.__posthogConfig) return;
    if (granted) {
